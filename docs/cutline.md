...
- [ ] `exportClaudeCodeSettings`, `GET /policy-versions/{id}/exports/claude-code`. Implemented as a read-only reference fragment (recorded in ACR-0018): only Rules with a deterministic Claude Code form are exported, every other Rule is listed in `unmappedRules`, and Authority Diff does not deploy or enforce the fragment.
