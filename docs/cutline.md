...
9. Evidence report를 내려받습니다.
   두 version의 `contentHash`, 기간, 분석한 Action 수와 `none` 비율, transition 표, Action Effect가 그대로일 때의 operation-level widening 표, group과 Verdict, 검토자와 시각, 그리고 아래 고정 문구가 들어갑니다.
