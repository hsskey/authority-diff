...
Web styles(`apps/web`): Tailwind v4 utility는 JSX에만 쓴다. CSS 파일은 `apps/web/src/styles/app.css`만 허용하고 그 파일은 `@import`, `@theme`, `@layer base`만 담는다(`pnpm lint:css`). `.css` import는 `main.tsx`에서만 한다. `@import "tailwindcss"`로 Preflight를 켠다(ACR-0012). `app.css`에 source string literal이 이름으로 쓰지 않는 class selector가 있으면 `pnpm lint:css`가 실패한다. `@apply`와 새 CSS class는 쓰지 않는다. 같은 utility 조합이 세 번 반복되면 React component로 추출한다. 색과 spacing은 `@theme` token만 쓰고(`bg-surface`, `text-muted`) arbitrary color는 쓰지 않는다. dark mode는 `prefers-color-scheme`이 그 token을 `:root`에서 override한다. 화면이 바뀌는 PR에는 screenshot diff가 필요하다.
...
8. root script: typecheck, lint, format, format:check, lint:css, lint:prove, lint:boundaries, lint:boundaries:prove, check:evidence, test,
   check(= typecheck + lint + format:check + lint:css + lint:boundaries + lint:prove + check:evidence + test).
