...
The figures on them match the recorded journey: adoption 평가 34,490, 허용 8,217, 확인 필요 26,253, 차단 20, groups 21 + 2; Change Review 넓어진 132, Widening group 7, resultHash `55a0cb73…cd18` (`adoption-preview.md`, `gate2-replay.md`).
...
- Screen 1: with no baseline, the proposal applied to 34,490 past Actions gives 허용 8,217 (23.8%), 확인 필요 26,253 (76.1%), 차단 20; 23 groups are 미판정 and the Gate blocker keeps "최초 정책 채택" disabled.
- Screen 2: the baseline is the screen 1 version; only ask → allow changed, 132 Actions (확인 필요 26,253 → 26,121), in 7 critical fetch groups to `unknown_remote` across five programs, and the `gh` and `git` groups also move to `trusted_remote`; 7 unjudged groups keep "정책 변경 수락" disabled.
