...
The table below is the V1 metrics as collected on classifier 0.2.4 and is kept as the record of that collection.
Its hashes are not comparable with the 0.2.5 values above.
...
The analyzable share on that version rose because more commands were recognized, and part of that rise was auto-allow: a script named by a path inside the workspace is `partial`, and `allow_workspace_execute` allows it without reading it ([classifier-hardening-4.md](classifier-hardening-4.md)).

The `github.com/**` scene on classifier 0.2.4 was 9 fetch Actions (5 `git` from one repository, 4 `gh` to four others).
