...
`sh -c '<node> <entry> hook <event> 2>>$HOME/.authority/hook-errors.log || exit 0'`
...
The shell wrapper ensures module-load failures also exit 0; stderr is written only to `~/.authority/hook-errors.log`.
