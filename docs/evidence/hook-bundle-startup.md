...
When the running node lives in a versioned Homebrew Cellar directory, `install-hooks` pins `<node>` to a stable symlink (`<prefix>/bin/node`, the formula's `opt/<formula>/bin/node`, or `/usr/local/bin/node`) that resolves to the same binary, so `brew upgrade` cannot delete the path out from under every installed hook.
A node under a version manager is pinned to a shim as described below, and any other path keeps the running interpreter.
`authority install-hooks --print` reports the chosen path and the reason on a `node` line.

When the running node lives inside a version-manager folder (`~/.vite-plus/js_runtime/node/<version>`, `~/.volta/tools/image/node/<version>`, or `~/.nvm/versions/node/<version>`), `install-hooks` pins `<node>` to that manager's shim (`~/.vite-plus/bin/node`, `~/.volta/bin/node`, or nvm's `~/.nvm/current/bin/node`), which picks an installed version at run time.
Known limitation: when the shim is missing, `install-hooks` keeps the versioned path, and after that node version is removed the hook silently stops working until `install-hooks` is re-run.
