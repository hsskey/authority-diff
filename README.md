...
- **Approximate remote parsing.** A Remote Key comes from the command text and the remotes found on disk at import time. A named remote used outside the Action workspace stays unresolved and falls to `unknown_remote`. A git refspec such as `refs/heads/main` can be read as a remote URL: `git ls-remote origin refs/heads/main` also falls to `unknown_remote`, while `git push origin refs/heads/main` matches `protectedBranches` on the refspec branch and lands in `protected`. Both refspec Actions stay at `ask` and neither is allowed (`docs/evidence/classifier-limitations.md`, `docs/evidence/replay-limitations.md`).
