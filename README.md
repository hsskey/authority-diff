...
- **Approximate remote parsing.** A Remote Key comes from the command text and the remotes found on disk at import time. A named remote used outside the Action workspace stays unresolved, and a git refspec such as `refs/heads/main` can be read as a remote URL; both land in `unknown_remote` (`docs/evidence/classifier-limitations.md`, `docs/evidence/replay-limitations.md`).
