...
- `apps/web` styles: JSX에는 Tailwind v4 utility만 쓴다. CSS는 `apps/web/src/styles/app.css` 하나(`pnpm lint:css`)이고 `@import`, `@theme`, `@layer base`만 허용한다. Preflight, `@apply`, 새 CSS class는 쓰지 않는다. 같은 utility 조합이 세 번 반복되면 React component로 추출한다. 색은 `@theme` token만 쓰고(`bg-surface`, `text-muted`) arbitrary color는 금지한다. 화면이 바뀌는 PR에는 screenshot diff가 필요하다.
