...
- package를 추가하거나 다른 package를 import하기 전: `docs/design.md` 21장, `docs/adr/0010`, 경계 규칙(`tests/arch/*.arch.test.ts`)을 읽는다.
...
- 작업을 끝내기 전 `pnpm check`와 `pnpm lint:boundaries:prove`를 실행한다. 경계와 naming 규칙은 arch test(`tests/arch/`)로 강제되고 두 명령은 그 alias다. Node는 22를 쓴다.
